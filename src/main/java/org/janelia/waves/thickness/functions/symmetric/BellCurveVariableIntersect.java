...
public class BellCurveVariableIntersect implements
		SymmetricDifferentiableParameterizedFunction {

	public double value(double x, double[] parameters)
			throws FunctionEvaluationException {
		if ( parameters.length != 3 ) {
			throw new FunctionEvaluationException( parameters, "Accept exactly three parameters, got " + parameters.length );
			
		}
		
		double xCorr = x - parameters[0];
		return parameters[2] * Math.exp( - 0.5 * xCorr * xCorr / ( parameters[1] * parameters[1] ) );
	}
	...
	public double[] gradient(double x, double[] parameters)
		...
		result[2] = this.value( x, parameters ) / parameters[2];
	...
	public double[] inverse(double x, double[] parameters) throws DomainError {
		...
		double sqrt = Math.sqrt( - 2 * parameters[1] * parameters[1] * Math.log( x / parameters[2] ) );
